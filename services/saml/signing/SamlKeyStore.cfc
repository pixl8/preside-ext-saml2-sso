...
// PUBLIC API METHODS
	...
	public string function getFormattedX509Certificate( boolean multiline=false ) {
		var raw = ToBase64( getCert().getEncoded() );
	...
	public string function getFormattedPrivateKey( boolean multiline=false ) {
		var raw = ToBase64( getPrivateKey().getEncoded() );

		if ( !arguments.multiline ) {
			return raw;
		}

		var pk = "-----BEGIN PRIVATE KEY-----" & Chr( 10 );

...
<cfscript>
	...
	tabs.append({
		...
		, link   = ( args.tab == "providers" ) ? "" : event.buildAdminLink( linkTo="saml2ConsumerManagement" )
	});
	tabs.append({
		  id     = "settings"
		, icon   = "fa-cogs grey"
		, title  = translateResource( "saml2:consumer.tabs.settings" )
		, active = ( args.tab == "settings" )
		, link   = ( args.tab == "settings" ) ? "" : event.buildAdminLink( linkTo="saml2ConsumerManagement.settings" )
	});
	tabs.append({
		  id     = "meta"
		, icon   = "fa-code blue"
